...
// DrainTCP waits until every TCP connection in the server's netstack has
// completed its close handshake. It returns nil once drained, or ctx's error.
//
...
func (s *Server) DrainTCP(ctx context.Context) error {
	// Wait blocks until the endpoint closes or enters TIME-WAIT (EventHUp).
	// A waiter goroutine may
	...
	for _, ep := range tcpipStackOf(s.lb.ns).RegisteredEndpoints() {
		...
		if err := waitTCPEndpoint(ctx, tcpEP); err != nil {
			return err
		}
	}
	return nil
...
func waitTCPEndpoint(ctx context.Context, ep tcpWaiter) error {
	...
	case <-done:
		return nil
	case <-ctx.Done():
		return fmt.Errorf("%w: TCP endpoint in %s", ctx.Err(), tcp.EndpointState(ep.State()))
	}
}
